...
let SheetTag = 9999999

extension UIViewController {
    
    // This is an extension, we store our state at props/associated object.
    ...
    func sheetShow(width: CGFloat = 0.0, height: CGFloat = 0.0, keyboardAvoiding: Bool = false, direction: SheetDirection = .up, autoClose: Bool = true, animated: Bool = true, dismissable: Bool = true, didShow: (() -> Void)? = nil, didAutoClose: (() -> Void)? = nil) {
        if let window = UIApplication.shared.keyWindow {
            ...
            if let controller = objc_getAssociatedObject(window, &UIViewController.sheetControllerProp) as? UIViewController {
                controller.sheetClose { [self] in
                    self.sheetShowInternal(width: width, height: height, keyboardAvoiding: keyboardAvoiding, direction: direction, autoClose: autoClose, animated: animated, dismissable: dismissable, didShow: didShow, didAutoClose: didAutoClose)
                }
            } else {
                self.sheetShowInternal(width: width, height: height, keyboardAvoiding: keyboardAvoiding, direction: direction, autoClose: autoClose, animated: animated, dismissable: dismissable, didShow: didShow, didAutoClose: didAutoClose)
            }
    ...
    func sheetShowInternal(width: CGFloat = 0.0, height: CGFloat = 0.0, keyboardAvoiding: Bool = false, direction: SheetDirection = .up, autoClose: Bool = true, animated: Bool = true, dismissable: Bool = true, didShow: (() -> Void)? = nil, didAutoClose: (() -> Void)? = nil) {
        ...
        if let window = UIApplication.shared.keyWindow {
            ...
            if cy == 0.0 {
                ...
                let cymax = UIScreen.main.bounds.height * 0.95
    ...
    func sheetClose(didClose: (() -> Void)? = nil) {
    ...
    static func getShowingSheet() -> UIViewController? {
        if let window = UIApplication.shared.keyWindow {
            let overlay = window.viewWithTag(SheetTag)
            let firstView = overlay?.subviews.first
            return firstView?.parentViewController
        }
        return nil
    }
    ...
    @objc func sheetRespondToPanGesture(recognizer: UIPanGestureRecognizer) {
        ...
        if recognizer.state == .ended || recognizer.state == .failed || recognizer.state == .cancelled {
            ...
            if direction == .up {
                if (self.view.frame.origin.y + self.view.frame.size.height) > (r.size.height+snap) {
                    sheetClose {
                        if let didAutoClose = objc_getAssociatedObject(self, &UIViewController.sheetDidAutoCloseProp) as! (() -> Void)? {
                            didAutoClose()
                        }
            ...
            if direction == .right {
                if (self.view.frame.origin.x + self.view.frame.size.width) < (self.view.frame.size.width-snap) {
                    sheetClose {
                        if let didAutoClose = objc_getAssociatedObject(self, &UIViewController.sheetDidAutoCloseProp) as! (() -> Void)? {
                            didAutoClose()
                        }
